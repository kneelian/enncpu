...
int CPU::EXECUTE(const INSN insn)
{
	...
	switch(insn.OPERATION)
	{
		...
		case HALFA: 
			ACTIVE_SET.at(insn.FIRST_REG) = (ACTIVE_SET.at(insn.SECOND_REG) >>  0) & 0xfff;
			break;
		case HALFB: 
			ACTIVE_SET.at(insn.FIRST_REG) = (ACTIVE_SET.at(insn.SECOND_REG) >> 12) & 0xfff;
			break;
		...
		case SWPR:
			temp = ACTIVE_SET.at(insn.FIRST_REG);
			ACTIVE_SET.at(insn.FIRST_REG) = SHADOW_SET.at(insn.FIRST_REG);
			SHADOW_SET.at(insn.FIRST_REG) = temp;
			break;
			
		case SHDW:
			SHADOW_SET[insn.FIRST_REG] = ACTIVE_SET[insn.FIRST_REG];
			break;
			
		case LITE:
			ACTIVE_SET[insn.FIRST_REG] = SHADOW_SET[insn.FIRST_REG];
			break;

		case SWAP:
		{
			std::array<u32, 8> TEMP_SET;
			for(int i = 0; i < 8; i++)
			{
				TEMP_SET[i] = ACTIVE_SET[i];
				ACTIVE_SET[i] = SHADOW_SET[i];
				SHADOW_SET[i] = TEMP_SET[i];
			}
		}
			break;
